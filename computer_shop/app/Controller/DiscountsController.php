...
class DiscountsController extends AppController {
	var $uses = array('User', 'Discount', 'Page');
	var $thisModel = 'Discount';
	var $thisController = 'discounts';
	...
	public function admin_add() {
        ...
        if ($this->request->is('post') || $this->request->is('put')) {
			...
			$startDate = (strtotime($data[$model]['start_date']));
			$endDate = (strtotime($data[$model]['end_date']));
			$currentTime = time();
			if ($startDate === '' || $endDate==='' || $startDate>$endDate || $endDate < $currentTime) { 
				$this->Flash->success(__('You must set correct dates')); 
				$this->redirect($this->referer());
			}
			$data[$model]['start_date'] = date("Y-m-d H:i:s", $startDate);
			$data[$model]['end_date'] = date("Y-m-d H:i:s", $endDate);
